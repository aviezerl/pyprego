...
# ── Config ──────────────────────────────────────────────────────────────
DEV_REMOTE="private"        # aviezerl/pyprego
MAIN_REMOTE="origin"        # tanaylab/pyprego
DEV_BRANCH="dev"
MAIN_BRANCH="main"
DEV_ONLY_PATHS=("dev/" "CLAUDE.md" "AGENTS.md" ".a5c/" "DECISIONS.md" "PROGRESS.md")  # removed from main after merge
...
git rev-parse --verify "$MAIN_BRANCH" >/dev/null 2>&1 \
    || die "Branch '$MAIN_BRANCH' does not exist."

# ── Lint check ─────────────────────────────────────────────────────────
if command -v ruff >/dev/null 2>&1; then
    info "Running ruff lint check..."
    ruff check pyprego/ || die "Ruff lint check failed. Fix issues before shipping."
else
    info "Warning: ruff not found, skipping lint check."
fi
...
if $DO_PUSH; then
    ...
    if command -v gh >/dev/null 2>&1; then
        if gh release view "$TAG" >/dev/null 2>&1; then
            ...
            NOTES="$(awk '/^## v'"${VERSION}"'/{found=1; next} /^## /{if(found) exit} found{print}' CHANGELOG.md 2>/dev/null || true)"
            if [[ -n "$NOTES" ]]; then
